...
class Configuration:
    ...
    def __init__(self):
        ...
        # --------- RUN NAME ---------
        self.run_name = 'RapidEye_China256_500Epoch_nonorm'                     # custom name for this run, eg resampled_x3, alpha60, new_train etc

        # ---------- PATHS -----------
        # Path to training areas and polygons shapefiles
        self.training_data_dir = '/mnt/sddb/UrbanTree/Tree_ROI/'
        self.training_area_fn = 'Rectangle_evalulation_GF_BJ1.gpkg'
        self.training_polygon_fn = 'Tree_evalulation_GF_BJ_RapidEye.gpkg'
        
        #self.training_area_fn = 'Ploygon_Train_20220228_RapidEye.shp'
        #self.training_polygon_fn = 'Tree_Train_20220228_RapidEye.shp'
        
        # Path to training images
        self.training_image_dir = '/mnt/sddb/UrbanTree/RapidEye/UrbanTree_RapideEye2010/original/' #'/nfs/Planet/PlanetScope/Mosaics/Africa/2019' 

        # Output base path where all preprocessed data folders will be created
        self.preprocessed_base_dir = '/mnt/sddb/UrbanTree/Evaluation_Images/RapidEye/'
        ...
        #self.continue_model_path = '/nfs/Users/Martin/RE/models/20220601-2137_RE_india256_nonorm.h5'            # If set to None, a new model is created
        self.continue_model_path = None

        # Path where trained models and training logs will be stored
        self.saved_models_dir = '/mnt/sddb/UrbanTree/Planet/Model/'
        self.logs_dir = '/mnt/sddb/UrbanTree/Planet/logs/'

        # Paths to input images to be predicted
        self.to_predict_dir = '/mnt/sddb/UrbanTree/PlanetImages_UrbanRegions_Clip/' #'/nfs/Planet/RapidEye/Mosaics/South_Asia/2010/not_matched'
        self.to_predict_filelist = ''            # [optional] path of a file with list of image paths to be predicted

        # Output path where predictions will be saved
        self.predictions_base_dir = '/mnt/sddb/UrbanTree/Planet/Prediction/' # '/nfs/Users/Martin/RE/results_planet'

        # Path to model to use for this prediction [optional]
        self.trained_model_path = None # '/nfs/Users/Martin/RE/models/Planet/backup/20220530-2250_Planet_india512.h5'            # if set to None, it will use the most recent trained model
        ...
        # ----- PREDICTION CONFIG ------
        self.prediction_workers = 2                # number of prediction processes predicting image chunks in parallel
        self.prediction_gridsize = (5, 3)          # num rows/cols in which images are split for parallel processing
        self.prediction_batch_size = 6           # Depends on GPU memory, patch size and num parallel workers
        self.prediction_stride = 192               # stride = width -> no overlap, stride = width/2 -> 50 % overlap
        self.prediction_threshold = 0.6            # threshold applied when converting float predictions to binary
...
# =============================================================================
    ...
    def validate(self):
        ...
        if self.predict_images_file_type not in [".tif", ".jp2", ".vrt"]:
