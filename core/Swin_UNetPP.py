# core/Swin_UNetPP.py
# PyTorch implementation aligned with the TensorFlow version:
# - Same window partition/reverse logic and attention mask construction
# - LayerNorm(eps=1e-5) over channels (per-pixel), via LayerNorm2d wrapper
# - MLP uses 1x1 convs (equivalent to Dense on channel-last)
# - PatchMerging / PatchExpansion and FinalPatchExpansion mirror TF math
# - Clamps and DropPath behavior match TF
# - Head outputs sigmoid like TF
...
def window_reverse(
    ...
    Args:
        ...
        window_size: (Wh, Ww)
        h, w, c: output spatial and channel dims.
    Returns:
        x: (B, C, H, W)
    """
    wh, ww = window_size
...
# ----------------------------
# Norm helper: LayerNorm over channels for 2D maps
# ----------------------------
class LayerNorm2d(nn.Module):
    """
    Keras LayerNormalization(eps=1e-5) over channels for (N, C, H, W):
    apply LN on the channel axis per pixel.
    """
...
class WindowAttention(nn.Module):
    def __init__(
        ...
        self.register_buffer("relative_position_index", rel_index.long(), persistent=False)
...
class SwinTransformerBlock(nn.Module):
    def __init__(
        ...
        # Attention mask for shifted windows (match TF construction)
        if self.shift_size > 0:
            ...
            attn_mask = torch.where(attn_mask != 0, torch.tensor(-1e9), torch.tensor(0.0))
            self.register_buffer("attn_mask", attn_mask.to(torch.float32), persistent=False)
        else:
...
class PatchEmbedding(nn.Module):
    def __init__(self, in_ch: int, embed_dim: int, patch_size: int) -> None:
        super().__init__()
        self.proj = nn.Conv2d(in_ch, embed_dim, kernel_size=patch_size, stride=patch_size, padding=0, bias=True)
...
class PatchMerging(nn.Module):
    ...
    Mirrors TF: reshape to 4*C channels, LayerNorm, Dense(2C) w/ no bias.
    """
...
class PatchExpansion(nn.Module):
    ...
    (TF: Dense(2*dim) + space-to-depth^(-1))
    """

    def __init__(self, dim: int) -> None:
        super().__init__()
        self.expand = nn.Conv2d(dim, 2 * dim, kernel_size=1, bias=False)
        self.shuffle = nn.PixelShuffle(upscale_factor=2)    # channels -> (2*dim)/4 = dim//2
...
class FinalPatchExpansion(nn.Module):
    ...
    (TF: Dense((up^2)*dim) -> reshape -> transpose -> reshape)
    """
    ...
    def __init__(self, dim: int, up_factor: int) -> None:
        ...
        self.shuffle = nn.PixelShuffle(upscale_factor=up_factor)  # -> dim channels
...
class SwinBlock(nn.Module):
    def __init__(
        ...
        self.swtb1 = SwinTransformerBlock(
            dim=dims, input_resolution=ip_res, num_heads=num_heads,
            window_size=window_size, shift_size=0,
            attn_drop_rate=attn_drop, proj_drop_rate=proj_drop,
            mlp_drop_rate=mlp_drop, drop_path_rate=drop_path,
        )
        self.swtb2 = SwinTransformerBlock(
            dim=dims, input_resolution=ip_res, num_heads=num_heads,
            window_size=window_size, shift_size=ss_size,
            attn_drop_rate=attn_drop, proj_drop_rate=proj_drop,
            mlp_drop_rate=mlp_drop, drop_path_rate=drop_path,
        )
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.enc_swin_blocks = nn.ModuleList(
            [
                nn.Sequential(
                    SwinBlock(c, (h, w), window_size=ws, ss_size=ss,
                              num_heads=max(1, c // 32),
                              attn_drop=attn_drop, proj_drop=proj_drop, mlp_drop=mlp_drop, drop_path=drop_path),
                    SwinBlock(c, (h, w), window_size=ws, ss_size=ss,
                              num_heads=max(1, c // 32),
                              attn_drop=attn_drop, proj_drop=proj_drop, mlp_drop=mlp_drop, drop_path=drop_path),
                ),
                nn.Sequential(
                    SwinBlock(2 * c, (h // 2, w // 2), window_size=ws, ss_size=ss,
                ...
                nn.Sequential(
                    SwinBlock(4 * c, (h // 4, w // 4), window_size=ws, ss_size=ss,
...
class Decoder(nn.Module):
    def __init__(
        ...
        # After concat with encoder features, reduce back to stage dims
    ...
    def forward(self, x: torch.Tensor, encoder_features: Sequence[torch.Tensor]) -> torch.Tensor:
        for patch_expand, swin_blocks, enc_ftr, skip_conv in zip(
            self.dec_patch_expand_blocks, self.dec_swin_blocks, reversed(encoder_features), self.skip_conv_layers
        ):
...
class SwinUNet(nn.Module):
    def __init__(
        ...
        self.encoder = Encoder(
            c, enc_ip,
            window_size=window_size, ss_size=ss_size,
            attn_drop=attn_drop, proj_drop=proj_drop, mlp_drop=mlp_drop, drop_path=drop_path
        )

        # Bottleneck dims / resolution
        bot_dim = c * (2 ** num_blocks)
        bot_ip = (h // (patch_size * (2 ** num_blocks)),
