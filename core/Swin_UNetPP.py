...
def window_reverse(windows, window_size, H, W):
    ...
    x = tf.reshape(windows, [B, tf.math.floordiv(H, window_h), tf.math.floordiv(W, window_w),
                             window_h, window_w, -1])
    x = tf.transpose(x, [0, 1, 3, 2, 4, 5])
...
class SwinTransformerBlock(layers.Layer):
    def __init__(self, dim, input_resolution, num_heads=4,
        ...
        if self.shift_size > 0:
            ...
            mask_windows = window_partition(tf.convert_to_tensor(img_mask), (self.window_size, self.window_size))
...
class FinalPatchExpansion(layers.Layer):
    ...
    def call(self, x):
        ...
        x = tf.reshape(x, [B, H, W, self.up_factor, self.up_factor, new_C])
        x = tf.transpose(x, [0, 1, 3, 2, 4, 5])
...
class SwinBlock(layers.Layer):
    def __init__(self, dims, ip_res, window_size=4, ss_size=None,
                 num_heads=4, attn_drop=0.0, proj_drop=0.0, mlp_drop=0.0, drop_path=0.1):
        super().__init__()
        if ss_size is None:
        ...
        self.swtb2 = SwinTransformerBlock(dim=dims, input_resolution=ip_res, num_heads=num_heads,
                                          window_size=window_size, shift_size=ss_size,
                                          attn_drop_rate=attn_drop, proj_drop_rate=proj_drop,
                                          mlp_drop_rate=mlp_drop, drop_path_rate=drop_path)

    def call(self, x, training=None):
        x = self.swtb1(x, training=training)
        x = self.swtb2(x, training=training)
        return x


class Encoder(models.Model):
    def __init__(self, C, input_resolution, window_size=4, ss_size=None,
                 attn_drop=0.0, proj_drop=0.0, mlp_drop=0.0, drop_path=0.1):
        super().__init__()
        H, W = input_resolution
        ws = window_size
        ...
        self.enc_swin_blocks = [
            keras.Sequential([
                SwinBlock(C,     (H,     W),     window_size=ws, ss_size=ss, num_heads=max(1, C//32),
                          attn_drop=attn_drop, proj_drop=proj_drop, mlp_drop=mlp_drop, drop_path=drop_path),
                SwinBlock(C,     (H,     W),     window_size=ws, ss_size=ss, num_heads=max(1, C//32),
                          attn_drop=attn_drop, proj_drop=proj_drop, mlp_drop=mlp_drop, drop_path=drop_path)
            ]),
            keras.Sequential([
                SwinBlock(2*C,   (H//2,  W//2),  window_size=ws, ss_size=ss, num_heads=max(1, (2*C)//32),
                          attn_drop=attn_drop, proj_drop=proj_drop, mlp_drop=mlp_drop, drop_path=drop_path),
                SwinBlock(2*C,   (H//2,  W//2),  window_size=ws, ss_size=ss, num_heads=max(1, (2*C)//32),
                          attn_drop=attn_drop, proj_drop=proj_drop, mlp_drop=mlp_drop, drop_path=drop_path)
            ]),
            keras.Sequential([
                SwinBlock(4*C,   (H//4,  W//4),  window_size=ws, ss_size=ss, num_heads=max(1, (4*C)//32),
                          attn_drop=attn_drop, proj_drop=proj_drop, mlp_drop=mlp_drop, drop_path=drop_path),
                SwinBlock(4*C,   (H//4,  W//4),  window_size=ws, ss_size=ss, num_heads=max(1, (4*C)//32),
                          attn_drop=attn_drop, proj_drop=proj_drop, mlp_drop=mlp_drop, drop_path=drop_path)
            ])
...
class Decoder(models.Model):
    def __init__(self, C, input_resolution, window_size=4, ss_size=None,
        ...
        self.dec_patch_expand_blocks = [PatchExpansion(8 * C), PatchExpansion(4 * C), PatchExpansion(2 * C)]
...
class ConvStem(layers.Layer):
    def __init__(self, out_channels, kernel_size=7, stride=4, **kwargs):
        super().__init__(**kwargs)
        self.conv = layers.Conv2D(out_channels, kernel_size=kernel_size, strides=stride,
...
class SwinUNet(models.Model):
    def __init__(self, H, W, ch, C, num_class=1, num_blocks=3, patch_size=16,
                 window_size=4, ss_size=None, attn_drop=0.0, proj_drop=0.0, mlp_drop=0.0, drop_path=0.1):
        super().__init__()
        ...
        self.bottleneck = SwinBlock(
            ...
            window_size=window_size, ss_size=ss_size,
            num_heads=max(1, (C * (2 ** num_blocks)) // 32),
            attn_drop=attn_drop, proj_drop=proj_drop, mlp_drop=mlp_drop, drop_path=drop_path
        )
    ...
    def call(self, x, training=None):
        ...
        x, skip_ftrs = self.encoder(x, training=training)
        x = self.bottleneck(x, training=training)
        x = self.decoder(x, skip_ftrs, training=training)
