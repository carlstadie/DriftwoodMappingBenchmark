...
def set_global_seed(seed: Optional[int] = None):
    ...
    Ensures deterministic behavior during experiments. If seed is None,
    function becomes a non operational so callers can remain concise.
    """
    if seed is None:
        return
...
class ModelEMA:
    ...
    Tracks a smoothed copy of weights for more stable evaluation and
    checkpointing without altering the training step.
    """
    ...
    @torch.no_grad()
    def update(self):
        """Update EMA weights using the current parameters."""
    ...
    def use_ema_weights(self, model: nn.Module):
        """
        Temporarily swap model parameterss with EMA weights.

        Useful for validation or best-checkpoint eval without persisting the swap.
        """
...
def _unpad(y: torch.Tensor, pad_h: int, pad_w: int) -> torch.Tensor:
    """Remove right/bottom padding added by _autopad."""
    if pad_h or pad_w:
...
def _forward_with_autopad(model: nn.Module, x: torch.Tensor) -> torch.Tensor:
    ...
    Basically ensuring shape constraints are met when needed.
    """
...
def _ensure_probabilities(y_pred: torch.Tensor) -> torch.Tensor:
    ...
    Else assume probabilities and clamp to [0,1]. Stupid but practical. Thanks Terramind...
    """
    with torch.no_grad():
...
def _ensure_nchw(t: torch.Tensor) -> torch.Tensor:
    """Accept [N,H,W] -> [N,1,H,W]; keep [N,C,H,W] as-is, bacuae terramind drops the channel dim for labels."""
    if t.dim() == 3:
...
def _is_terramind_model(model: nn.Module) -> bool:
    """
    Detect TerraMind even when wrapped by torch.compile.
...
def _as_probs_from_terratorch_logits_first(
    out,
    num_classes: int = 1,
) -> torch.Tensor:
    """
    TerraTorch containers --> probability maps. Prefer logits (keeps grad),
    then float predictions. Always return float NCHW.
    """
    ...
    # Allow prediction ONLY if it's already a float prob map.
    ...
    # Fallbacks (dict values, attrs, sequences, tensors). No idea which is used :(
...
def _as_probs_from_terratorch(out, num_classes: int = 1) -> torch.Tensor:
    """
    Turn any unknown TerraTorch ModelOutput / HF-style container / dict / tuple / tensor --> probability maps.
    ...
    Order of attempts:
      ...
      Technically should be a tensor as autput of segmentation task, but not well documented
    """
    from collections.abc import Mapping
    import torch as _torch
    ...
    #  Mapping-like (many ModelOutput types behave like dicts).
    if isinstance(out, Mapping):
    ...
    #  Adapters common in custom ModelOutput classes.
