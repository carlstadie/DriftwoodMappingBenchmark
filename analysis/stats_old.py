...
def fit_factorial_model_with_bf(
    ...
    # effect baselines for coefficient interpretation under effects coding
    d0_name = EFFECTS_BASELINE_DATASET
    a0_name = EFFECTS_BASELINE_ARCH
    if d0_name not in datasets:
        d0_name = datasets[-1]
    if a0_name not in archs:
        a0_name = archs[-1]
    d0 = datasets.index(d0_name)
    a0 = archs.index(a0_name)
    ...
    # Dataset coefficients: dataset vs baseline dataset (marginal over arch)
    for d_name in list(dcat.categories)[:-1]:
        if d_name not in datasets:
            continue
        di = datasets.index(d_name)
        delta_t = dataset_marg_t[:, di] - dataset_marg_t[:, d0]
        delta_o = dataset_marg_o[:, di] - dataset_marg_o[:, d0]
        base_o = dataset_marg_o[:, d0]
        _write_effect(f"beta_dataset[{d_name}]", delta_t, delta_o, base_o)

    # Arch coefficients: arch vs baseline arch (marginal over dataset)
    for a_name in list(acat.categories)[:-1]:
        if a_name not in archs:
            continue
        ai = archs.index(a_name)
        delta_t = arch_marg_t[:, ai] - arch_marg_t[:, a0]
        delta_o = arch_marg_o[:, ai] - arch_marg_o[:, a0]
        base_o = arch_marg_o[:, a0]
        _write_effect(f"beta_arch[{a_name}]", delta_t, delta_o, base_o)

    # Interaction coefficients: diff-in-diff vs (baseline dataset, baseline arch)
    for d_name in list(dcat.categories)[:-1]:
        for a_name in list(acat.categories)[:-1]:
            if d_name not in datasets or a_name not in archs:
                continue
            di = datasets.index(d_name)
            ai = archs.index(a_name)

            delta_t = (pred_draws_transformed[:, di, ai] - pred_draws_transformed[:, di, a0]) - (
                pred_draws_transformed[:, d0, ai] - pred_draws_transformed[:, d0, a0]
            )
            delta_o = (pred_draws_original[:, di, ai] - pred_draws_original[:, di, a0]) - (
                pred_draws_original[:, d0, ai] - pred_draws_original[:, d0, a0]
            )

            base_o = np.zeros_like(delta_o)  # baseline for interaction is 0
